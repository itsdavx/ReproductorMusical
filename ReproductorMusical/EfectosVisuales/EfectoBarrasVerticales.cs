    ...
    public class EfectoBarrasVerticales : IEfectoVisual
    {
        ...
        public void Renderizar(Graphics g, int ancho, int alto, float[] buffer)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (objetivo > alturaMax) objetivo = alturaMax;
                ...
                if (beat && ratio > 0.5f)
                    colorTop = Color.FromArgb(100, 180, 255);
                else if (ratio > 0.75f)
                    colorTop = Color.FromArgb(255, (int)(80 * (1 - ratio)), 40);
                else if (ratio > 0.4f)
                    colorTop = Color.FromArgb(255, (int)(200 * ratio), 0);
                else
                    colorTop = Color.FromArgb(50, 220, 80);
                colorBot = Color.FromArgb(colorTop.R / 4, colorTop.G / 4, colorTop.B / 4);
